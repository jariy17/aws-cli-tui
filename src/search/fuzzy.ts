...
function subsequenceScore(key: string, query: string): number | undefined {
  let queryIndex = 0;
  let firstMatch = -1;
  let previousMatch = -1;
  let gaps = 0;
  ...
  for (
    let keyIndex = 0;
    keyIndex < key.length && queryIndex < query.length;
    keyIndex += 1
  ) {
    if (key[keyIndex] !== query[queryIndex]) continue;
    if (firstMatch < 0) firstMatch = keyIndex;
    if (previousMatch >= 0) gaps += keyIndex - previousMatch - 1;
...
export function scoreNormalizedFuzzyText(
  key: string,
  query: string,
): number | undefined {
  if (!key || !query) return undefined;
  if (key === query) return 0;
  if (key.startsWith(query)) return 100 + key.length - query.length;
  ...
  if (query.length >= 3) return subsequenceScore(key, query);
  return undefined;
}
...
export function scoreFuzzyText(
  ...
  return scoreNormalizedFuzzyText(
    normalizeFuzzyText(candidate),
    normalizeFuzzyText(query),
  );
}
