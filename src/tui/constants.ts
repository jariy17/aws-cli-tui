export const HELP = {
  search:
    "service:x · resource:x · ↑/↓ Move · ←/→ Page · Enter Open · Esc Clear/Exit",
  pages: "/ Filter · ↑/↓ Move · ←/→ Page · Enter Open · Esc Clear/Back",
  configurablePages:
    "/ Filter · ↑/↓ Move · ←/→ Page · Enter Open · E Inputs · Esc Clear/Back",
  detail:
    "/ Filter · ↑/↓ Move · ←/→ Page · Enter Open · R Related · Esc Clear/Back",
  related: "/ Filter · ↑/↓ Move · ←/→ Page · Enter Open · Esc Clear/Back",
  input: "Type value · Enter Continue · Esc Back",
  enumInput: "/ Filter · ↑/↓ Move · ←/→ Page · Enter Select · Esc Clear/Back",
  configureInput: "Type value · Enter Continue/Run · Esc Back",
  configureEnum:
    "/ Filter · ↑/↓ Move · ←/→ Page · Enter Select/Run · Esc Clear/Back",
} as const;

export const COLORS = {
  accent: "cyan",
  action: "yellow",
  success: "green",
  error: "red",
  muted: "gray",
} as const;
